...
static XLink_sem_t* getAndRefSem(pthread_t threadId, xLinkSchedulerState_t *curr, int inc_ref);
static int unrefSem(XLink_sem_t* sem,  xLinkSchedulerState_t* curr);
...
XLinkError_t DispatcherInitialize(DispatcherControlFunctions *controlFunc) {
    ...
    if (XLink_sem_init(&addSchedulerSem, 0, 1)) {
...
XLinkError_t DispatcherStart(xLinkDeviceHandle_t *deviceHandle)
{
    ...
    XLink_sem_wait(&addSchedulerSem);
    ...
    XLink_sem_post(&addSchedulerSem);
...
int DispatcherWaitEventComplete(xLinkDeviceHandle_t *deviceHandle)
{
    ...
    XLink_sem_t* id = getAndRefSem(pthread_self(), curr, 0);
    if (id == NULL) {
        return -1;
    }
...
static XLink_sem_t* createSem(xLinkSchedulerState_t* curr)
{
    ...
    XLink_sem_t* sem = getAndRefSem(pthread_self(), curr, 0);
        ...
        while (temp < curr->eventSemaphores + MAXIMUM_SEMAPHORES) {
            if (temp->refs < 0 || curr->semaphores == MAXIMUM_SEMAPHORES) {
                if (curr->semaphores == MAXIMUM_SEMAPHORES && !temp->refs) {
                    XLINK_RET_ERR_IF(XLink_sem_destroy(&temp->sem) == -1, NULL);
                    curr->semaphores --;
                    temp->refs = -1;
#if (defined(_WIN32) || defined(_WIN64))
